...
記述だけを切り出した `Renderable` の struct（[コンポーネント](components.md#renderable-で記述を分割する)）は、この所有関係に何も足しません。レンダリング 1 回分しか生きず、受け取ったクロージャは呼び出し元の記述と同じ扱いになります。`ToDoRow(item:) { self.toggle(item) }` のように content の `self` をキャプチャして構いません — 循環しない理由は上と同じです。

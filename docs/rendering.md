...
> ⚠️ **配列式にはスロットが付きません。** `items.map { ... }` や `[a, b] + [c]` が生む子は素の並びとして扱われるため、**配列が縮むと後ろの兄弟が繰り上がります**。長さの変わる並びの後ろに兄弟を置く場合は `for-in` で書いてください。
>
> ```swift
> FineStack.vertical {
>     for item in self.items {        // ✅ スロットが付く
...
> ```
>
> これは「まだ実装していない」ではなく、**現在のスロット模型では両立しない**ためです。配列式にスロットを与えると、次の2つが同時には満たせなくなります(実測で確認済み)。
>
> 1. `if flag { [a] } else { b }` の 2 分岐が同じスロットを共有すること — 配列の要素と素の子が同じ位置に並ぶ必要があり、run が位置を消費しなければなりません
> 2. `if { for ... ; footer }` の footer が run の長さに影響されないこと — run が位置を消費**しない**必要があります
>
> 加えて `@FineBuilder` を付けたヘルパーを式として使う(`FineStack.vertical { self.helper(flag) }`)と、ヘルパー内の素の子が配列の添字で識別されてしまい、ヘルパー内の条件が消えた時点で identity が変わります。
>
> 解くにはスロットの代数そのものを設計し直す必要があり、パッチでは届きません。
